...
def run_test(args):
    ...
    all_df.columns = ['id','sg','pj','db','zb','qsht']
    '''
    ...
    all_df['db'] = all_df['db'].apply(lambda x: 1 if x>0.5 else 0)
    all_df['zb'] = all_df['zb'].apply(lambda x: 1 if x>0.5 else 0)
    all_df['qsht'] = all_df['qsht'].apply(lambda x: 1 if x>0.5 else 0)
